...
#' @export
#'
pnova <- function(dataset=NULL, dist='bray', weighted=F, allFactors=T, factors=NULL) {
  if(is.null(dataset)) dataset <- get('active_dataset',envir = mvEnv)
  ...
  groupings <- unlist(lapply(factors, function(x) dst_results$metadata[x]),recursive = F)

  formula <- as.formula(paste('dst ~ ',paste0('groupings$',names(groupings),collapse = '+')))

  dst_stats <- adonis(formula)
