...
plotBetaDiv <- function(dataset=NULL,
  ...
  lab.xpos <- 1.5*max(p$data$Axis.1)
  lab.ypos <- 1.5*max(p$data$Axis.2)
  ...
  if(showStats) {
    ...
    p <- p+coord_cartesian(xlim=c(x_range[[1]], x_range[[2]]+0.2*(x_range[[2]]-x_range[[1]])),
                           ylim=c(y_range[[1]], y_range[[2]]+0.2*(y_range[[2]]-y_range[[1]])),
                           clip='off')+
      annotation_custom(grob = textGrob(label=r2,hjust=0.5,gp=gpar(cex=2)),
                        ymin=1.2*y_range[[2]],ymax=1.2*y_range[[2]],
                        xmin=x_range[[2]],xmax=x_range[[2]])+
    ...
    #   annotation_custom(grob = textGrob(label=pval,hjust=0.5,gp=gpar(cex=2)),
    #                     ymin=.8*lab.ypos,ymax=.8*lab.ypos,
    #                     xmin=lab.xpos,xmax=lab.xpos)
  }
